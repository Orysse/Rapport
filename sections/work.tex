...
\medskip
\item[•] \textbf{\texttt{debug:config}} : environnement crée pour toute les configuration avec accès au bon cache seulement, permettant de reproduire exactement l'environnement d'un job spécifique, y compris sur \acrshort{gpu}.
\end{itemize}
\medskip
...
\medskip
\item \textbf{Performance} : le système de \gls{cache} à deux niveaux réduit significativement les temps d'exécution. Un build complet sans \gls{cache} prenait initialement 45--60 minutes ; avec \gls{cache}, ce temps est réduit de 8--12 minutes.
\medskip
