...
déploiement continus développé pour \gls{trust}. On y observe les différentes étapes
...
Ce tableau de bord \gls{grafana} offre une vue d'ensemble de l'activité des \glspl{runner}
\gls{gitlab} déployés sur l'infrastructure du laboratoire. Les métriques affichées
permettent de suivre l'état de la file d'attente des jobs, le taux
d'utilisation des \glspl{runner}, les temps d'exécution moyens et l'historique des
...
assurer la fiabilité du système de validation automatisée de \gls{trust}.

\subsection{Utilisation de la CI dans les MRs}
\label{ann:mr-ci}
\begin{figure}[H]
    \centering
...
\subsection{Ajustements pour suivre les besoins de l'équipe}
\label{ann:ci-fail}
\begin{figure}[H]
    \centering
